...
disease = {'alpha':0.6,'beta':0.1,'gamma':0.0} 		#alpha is the infect rate, beta is the recover rate, gamma death rate
...
def SIRoneDay(line):											#Calculate SIR for each country
	...
	newRecover = min(INo,newRecover)		
	SNo -= newInfect
	INo += (newInfect-newRecover)
	RNo += newRecover      
	TNo = SNo + INo + RNo + DNo
	[SRate,IRate,RRate,DRate] = [SNo*1.0/TNo,INo*1.0/TNo,RNo*1.0/TNo,DNo*1.0/TNo]
